"""Robust long-only optimization layers for the v4 benchmark portfolio diagnostics."""
...
class RobustOptimizerCache:
    def __init__(self, *, sigma_by_month: dict[pd.Timestamp, np.ndarray]) -> None:
        self.sigma_by_month = sigma_by_month
        self._layer_cache: dict[tuple[pd.Timestamp, float, float, str], tuple[cp.Problem, cp.Parameter, cp.Variable]] = {}
    ...
    def get_layer(self, month_end: pd.Timestamp, config: OptimizerConfig) -> tuple[cp.Problem, cp.Parameter, cp.Variable]:
        ...
        if key not in self._layer_cache:
            ...
            self._layer_cache[key] = (problem, mu, w)
        return self._layer_cache[key]

    def solve(self, month_end: pd.Timestamp, mu: np.ndarray, config: OptimizerConfig) -> np.ndarray:
        problem, mu_param, w_var = self.get_layer(month_end, config)
        mu_param.value = np.asarray(mu, dtype=float)
        problem.solve(solver=cp.ECOS, warm_start=True, verbose=False)
        if w_var.value is None:
            raise RuntimeError(f"Robust optimizer failed at month_end={month_end} with status={problem.status}")
