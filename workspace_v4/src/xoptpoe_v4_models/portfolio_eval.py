"""Portfolio-evaluation helpers for the v4 supervised benchmark comparison."""
...
from xoptpoe_v4_models.data import SLEEVE_ORDER, aggregate_horizon_values
from xoptpoe_v4_models.optim_layers import OptimizerConfig, RobustOptimizerCache
...
def run_portfolio_evaluation(
    *,
    ...
    for month_end, chunk in signal_panel.groupby("month_end", sort=True):
        ...
        mu_pred = ordered["y_pred"].to_numpy(dtype=float)
        model_weights = optimizer_cache.solve(pd.Timestamp(month_end), mu_pred, optimizer_config)
        eq_weights = np.repeat(1.0 / len(SLEEVE_ORDER), len(SLEEVE_ORDER))

        strategies = {"equal_weight": eq_weights, model_strategy_name: model_weights}
...
def summarize_portfolio_metrics(returns_df: pd.DataFrame) -> pd.DataFrame:
    required = {"split", "strategy", "month_end", "portfolio_annualized_excess_return", "turnover", "drawdown", "cum_nav"}
