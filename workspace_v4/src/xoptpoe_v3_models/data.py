"""Data loading helpers for XOPTPOE v3 neural modeling."""
...
from xoptpoe_v3_modeling.features import DEFAULT_FEATURE_SET, feature_columns_for_set
from xoptpoe_v3_modeling.io import load_csv, load_parquet
...
def _load_v3_sleeve_order() -> tuple[str, ...]:
    asset_master_path = _module_project_root() / "data" / "final_v3_long_horizon_china" / "asset_master.csv"
    asset_master = pd.read_csv(asset_master_path)
    sleeves = tuple(asset_master["sleeve_id"].astype(str).tolist())
    if "EQ_CN" not in sleeves:
        raise ValueError(f"v3 asset master does not contain EQ_CN: {asset_master_path}")
    return sleeves


SLEEVE_ORDER: tuple[str, ...] = _load_v3_sleeve_order()


@dataclass(frozen=True)
class ModelPaths:
    """Filesystem layout for the v3 neural modeling stage."""
    ...
    china_sleeve_monthly_returns: Path
    tb3ms_monthly: Path
    data_out_dir: Path
    reports_dir: Path
...
def default_paths(project_root: Path) -> ModelPaths:
    ...
    return ModelPaths(
        project_root=root,
        train_split=root / "data" / "modeling_v3" / "train_split.parquet",
        validation_split=root / "data" / "modeling_v3" / "validation_split.parquet",
        test_split=root / "data" / "modeling_v3" / "test_split.parquet",
        feature_manifest=root / "data" / "modeling_v3" / "feature_set_manifest.csv",
        asset_master=root / "data" / "final_v3_long_horizon_china" / "asset_master.csv",
        sleeve_monthly_returns=root / "data" / "intermediate" / "sleeve_monthly_returns.csv",
        china_sleeve_monthly_returns=root / "data" / "final_v3_long_horizon_china" / "china_sleeve_monthly_returns.csv",
...
def _sort_split_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if work["sleeve_id"].isna().any():
        raise ValueError("split frame contains sleeves outside the active v3 sleeve order")
...
def _load_monthly_excess_history(paths: ModelPaths) -> pd.DataFrame:
    ...
    china_returns_df = load_csv(paths.china_sleeve_monthly_returns, parse_dates=["month_end"])
    tb3ms = load_csv(paths.tb3ms_monthly, parse_dates=["month_end"])

    returns_df = pd.concat([returns_df, china_returns_df], ignore_index=True)
    ...
    if panel.columns.isna().any():
        raise ValueError("monthly excess history could not be aligned to the active v3 sleeve order")
