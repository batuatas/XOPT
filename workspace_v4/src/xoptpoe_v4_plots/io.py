...
def _load_total_return_panel(paths: PlotPaths) -> pd.DataFrame:
    # Reconstruct the total return panel exactly as the models saw it during V4 training,
    # rather than rebuilding from missing raw price histories.
    fm = load_parquet(paths.project_root / "data_refs" / "feature_master_monthly.parquet")
    work = fm[["month_end", "sleeve_id", "ret_1m_lag"]].dropna().copy()
    work["month_end"] = pd.to_datetime(work["month_end"])
    
    panel = (
        work.pivot(index="month_end", columns="sleeve_id", values="ret_1m_lag")
        .sort_index()
...
def _optimizer_cache(paths: PlotPaths, months: list[pd.Timestamp]) -> RobustOptimizerCache:
    # Use the same exact benchmark-equivalent fallback as the scenario runner
    fm = load_parquet(paths.project_root / "data_refs" / "feature_master_monthly.parquet")
    work = fm[["month_end", "sleeve_id", "ret_1m_lag", "short_rate_US"]].dropna().copy()
    work["month_end"] = pd.to_datetime(work["month_end"])
    work["rf_1m"] = work["short_rate_US"] / 1200.0
    work["excess_ret_1m"] = work["ret_1m_lag"] - work["rf_1m"]
    
    monthly_excess_history = work.pivot_table(
        index="month_end",
        columns="sleeve_id",
        values="excess_ret_1m",
        aggfunc="first",
    ).reindex(columns=list(SLEEVE_ORDER)).astype(float)
    
    if monthly_excess_history.empty:
