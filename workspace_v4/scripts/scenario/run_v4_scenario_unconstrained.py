...
def run_scenario() -> None:
    ...
    for anchor in ANCHOR_DATES:
        ...
        QUESTION_ANCHOR_MAP = {
            "Q3_house_view_7pct_total": "2023-12-31",
            "Q4_less_gold": "2024-12-31",
        }

        for q in questions:
            qid = q["question_id"]

            target_anchor = QUESTION_ANCHOR_MAP.get(qid)
            if target_anchor != str(anchor.date()):
                print(f"\n  Q: {qid} [SKIPPED: Only mapped to anchor {target_anchor}]")
                continue
    ...
    gate_df.to_csv(REPORTS / "scenario_anchor_truth_unconstrained_v4.csv", index=False)
    audit_df.to_csv(REPORTS / "scenario_anchor_audit_unconstrained_v4.csv", index=False)

    if not results_df.empty:
        results_df.to_csv(REPORTS / "scenario_results_unconstrained_v4.csv", index=False)
        print(f"  scenario_results_unconstrained_v4.csv ({len(results_df)} rows)")
    else:
        print("  WARNING: no results produced")
        return

    _write_regime_summary(results_df, REPORTS / "scenario_regime_summary_unconstrained_v4.csv")
    _write_selected_cases(results_df, REPORTS / "scenario_selected_cases_unconstrained_v4.csv")
    _write_question_manifest(results_df, REPORTS / "scenario_question_manifest_unconstrained_v4.csv")
    _write_portfolio_response_summary(results_df, REPORTS / "scenario_portfolio_response_summary_unconstrained_v4.csv")
    _write_regime_transition_summary(results_df, REPORTS / "scenario_regime_transition_summary_unconstrained_v4.csv")
    _write_selected_questions(results_df, REPORTS / "scenario_selected_questions_unconstrained_v4.csv")
    _write_mala_diagnostics(results_df, REPORTS / "scenario_mala_diagnostics_unconstrained_v4.csv")
...
def _build_questions(anchor, pipeline, m0, scales, var1_prior):
    ...
    # Q1: Keep current gold-favorable probe
    G1, grad1 = gold_weight_probe(
        pipeline, m0, scales,
        l2reg=0.05,
        prior=var1_prior, var1_l2reg=0.005,
        sign=-1.0,
    )
    questions.append({
    ...
    # Q2: Keep current active tilt / equal-weight deviation probe
    ...
    if pipeline.has_analytical_grad():
        _fd_eps_q2 = 1e-4

        def grad2_analytical(m: np.ndarray) -> np.ndarray:
            ...
            for k in range(n):
                ...
                active_p = float(w_p @ mu_p) - float(ew @ mu_p)
                active_n = float(w_n @ mu_n) - float(ew @ mu_n)
                dG_dmu[k] = -(active_p - active_n) / (2.0 * _fd_eps_q2)

            grad_task = dG_dmu @ J_mu
            grad_reg = var1_prior.regularizer_grad(m, m0, l2reg=0.1)
            return grad_task + grad_reg

        grad2 = grad2_analytical
    else:
    ...
    # Q3: Keep current house-style question, but soften the total-return hurdle a bit
    ...
    G3, grad3 = house_view_return_probe(
        pipeline, m0,
    ...
    G5, grad5 = more_diversification_probe(
        pipeline, m0, scales,
        l2reg=0.05,
        prior=var1_prior, var1_l2reg=0.005,
    )
    ...
    # Q6: Classic 60/40
    G6, grad6 = classic_sixty_forty_probe(
        pipeline, m0, scales,
    ...
    G7, grad7 = excess_return_target_probe(
        pipeline, m0,
    ...
    # Q8: More equity
    G8, grad8 = max_total_equity_probe(
        pipeline, m0, scales,
        l2reg=0.05,
        prior=var1_prior, var1_l2reg=0.005,
    )
    ...
    # Q9: Flight to Safety
    G9, grad9 = flight_to_safety_probe(
        pipeline, m0, scales,
        l2reg=0.05,
        prior=var1_prior, var1_l2reg=0.005,
    )
    ...
    G10, grad10 = real_asset_rotation_probe(
        pipeline, m0, scales,
        l2reg=0.05,
        prior=var1_prior, var1_l2reg=0.005,
    )
    ...
    # Q11: Max Sharpe
    G11, grad11 = max_sharpe_total_probe(
        pipeline, m0, scales,
        l2reg=0.05,
        prior=var1_prior, var1_l2reg=0.005,
    )
