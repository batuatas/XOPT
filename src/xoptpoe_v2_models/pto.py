"""Predict-then-optimize workflow for XOPTPOE v2 long-horizon SAA."""
...
def _render_pto_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# XOPTPOE v2 PTO Report")
    lines.append("")
    ...
    lines.append("- The paper’s firm-level 1m stock ranking step is removed; the investable universe is the fixed 8-sleeve XOPTPOE universe.")
    ...
    lines.append("- Risk estimation uses trailing monthly sleeve excess returns from the frozen v1 history, with an expanding-to-60m EWMA window instead of adding a new 60m sample burn-in that would collapse the long-horizon split." )
    lines.append("")
