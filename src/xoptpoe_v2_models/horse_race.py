...
from workspace_v4.src.xoptpoe_v2_modeling.io import load_csv, write_csv, write_parquet, write_text
from xoptpoe_v2_models.data import TARGET_COL, build_sleeve_horizon_benchmark, default_paths, load_modeling_inputs
...
from xoptpoe_v2_models.portfolio_eval import run_portfolio_evaluation, summarize_portfolio_metrics
from xoptpoe_v2_models.prediction_ablation import (
    HORIZON_MODES,
...
def _portfolio_run_from_signal(
    *,
    ...
    benchmark_metrics["avg_max_weight"] = 1.0 / 8.0
    benchmark_metrics["avg_hhi"] = 1.0 / 8.0
    benchmark_metrics["avg_effective_n_assets"] = 8.0
...
def _render_predictor_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# XOPTPOE v2 Predictor Horse Race Report")
    lines.append("")
    lines.append("## Scope")
...
def _render_portfolio_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# XOPTPOE v2 SAA Portfolio Comparison Report")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Predictor-driven portfolios use a common allocator design: long-only, fully invested, no leverage, default v2 risk model, validation-selected lambda/kappa/Omega from the existing compact grid.")
