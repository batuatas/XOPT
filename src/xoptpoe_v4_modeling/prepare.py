"""End-to-end first-pass modeling preparation for XOPTPOE v4_expanded_universe."""
...
from xoptpoe_v4_modeling.features import (
    DEFAULT_FEATURE_SET,
    FEATURE_SET_ORDER,
...
from xoptpoe_v4_modeling.splits import (
    EXCLUDED_SPLIT_NAME,
    SplitConfig,
...
def default_paths(project_root: Path) -> PrepPaths:
    ...
    return PrepPaths(
        project_root=root,
        source_panel=root / "data" / "final_v4_expanded_universe" / "modeling_panel_hstack.parquet",
        feature_dictionary=root / "data" / "final_v4_expanded_universe" / "feature_dictionary.csv",
        interaction_dictionary=root / "data" / "final_v4_expanded_universe" / "interaction_dictionary.csv",
...
def _build_report(*, filtered_panel, split_panel, split_manifest, split_summary, feature_set_summary):
    ...
    rows = []
    rows.append("# XOPTPOE v4_expanded_universe Modeling Preparation Report")
    rows.append("")
    rows.append("## Filtering Applied")
    rows.append("- Source table: `data/final_v4_expanded_universe/modeling_panel_hstack.parquet`.")
    ...
    rows.append("- Rationale: validation and test should contain both horizons and the full v4 cross-section when coverage permits.")
    ...
    rows.append("")
    rows.append("## Locked v4 Notes")
    rows.append("- `CR_US_IG` replaces legacy `FI_IG` naming in the v4 modeling-prep branch.")
    rows.append("- `RE_US` and `LISTED_RE` coexist as separate sleeves.")
    rows.append("- Euro fixed-income sleeves enter as USD-unhedged synthesized targets built from local ETF returns plus FX conversion.")
    return "\n".join(rows) + "\n"


def run_v4_modeling_preparation(*, project_root: Path, validation_months: int = 24, test_months: int = 24, min_train_months: int = 60):
    ...
    write_text(report_text, paths.reports_dir / "modeling_prep_v4_report.md")
    ...
    return {
        ...
        "modeling_prep_v4_report": paths.reports_dir / "modeling_prep_v4_report.md",
    }
