...
def graphic04_prediction_evidence_v4(ctx: PlotContext) -> FigureArtifact:
    ...
    _rounded_panel(ax_side, 0.05, 0.58, 0.88, 0.26, fc=OFF_WHITE, radius=0.04)
    ax_side.text(0.12, 0.78, "Test-set readout", fontsize=15.5, fontweight="bold", color=NEUTRAL_DARK, ha="left", va="center")
    ax_side.text(0.12, 0.68, f"Correlation\n{metric['test_corr']:.2f}", fontsize=18.5, fontweight="bold", color=NEUTRAL_DARK, ha="left", va="center")
    ax_side.text(0.55, 0.68, f"RMSE\n{100*metric['test_rmse']:.1f}%", fontsize=18.5, fontweight="bold", color=NEUTRAL_DARK, ha="left", va="center")
    ax_side.text(0.12, 0.51, f"Sign accuracy\n{metric['test_sign_accuracy']:.2f}", fontsize=12.8, color=NEUTRAL_MID, ha="left", va="center")

    handles = _legend_handles_from_sleeves(_ordered_sleeves(ctx))
    ax_side.legend(handles=handles, loc="lower left", bbox_to_anchor=(0.02, 0.02), frameon=False, ncol=1, handletextpad=0.4, labelspacing=0.75, borderaxespad=0)
